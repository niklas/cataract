class Maintenance::ContentPathCompressor < Maintenance::Base
  ...
  def work
    Torrent.where('content_path_infix IS NOT NULL').each do |t|
      if t.content_filenames.length == 1 && t.content_filenames.first == t.content_path_infix
        unless (t.content_directory.path/t.content_path_infix).exist?
          t.status = 'missing'
          t.content_path = nil
        ...
        # nuffin
      elsif t.content_filenames.first.start_with?(t.content_path_infix)
